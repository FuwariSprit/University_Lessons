...
void processing(void){
  ...
  for(i=0; i<height; i++){
    for(j=0; j<width; j++){
      ...
      // 拡大縮小
      x = (int)(reverse_rate * j + 0.5);
      y = (int)(reverse_rate * i + 0.5);
 
      if((x>=0 && x<= width-1) && (y>=0 && y<= height-1)){
	imgout[0][j][height-i-1] = imgin[0][x][height-y-1];
	imgout[1][j][height-i-1] = imgin[1][x][height-y-1];
	imgout[2][j][height-i-1] = imgin[2][x][height-y-1];
      } else {
	imgout[0][j][height-i-1] = 0;
	imgout[1][j][height-i-1] = 128;
	imgout[2][j][height-i-1] = 128;
      }
    }
