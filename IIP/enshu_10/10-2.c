...
void contraction(void){
  int i, j;
  unsigned char work[512][512];
  
  for(i=height-1; i>=0; i--){
    for(j=0; j<width; j++){
      work[j][i] = imgout[0][j][i];
    }
  }
  ...
  for(i=height-1; i>=0; i--){
    for(j=0; j<width; j++){
      if(imgout[0][j][i]==0){
	if(work[j-1][i-1]==0xFF || work[j-1][i]==0xFF|| work[j-1][i+1]==0xFF || work[j][i-1]==0xFF ||
	   work[j][i+1]==0xFF || work[j+1][i-1]==0xFF || work[j+1][i]==0xFF || work[j+1][i+1]==0xFF){
	  imgout[0][j][i]=0xFF;
	}
      }
    }
