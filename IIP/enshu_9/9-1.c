...
void processing(void){
  int i, j;
  double filter[3][3] = {{0.111111, 0.111111, 0.111111}, // フィルタ係数
			 {0.111111, 0.111111, 0.111111},
			 {0.111111, 0.111111, 0.111111}};
  int point_H = 0; // 水平
  int point_V = 0; // 垂直
  double a_imgin = 0; // フィルタ係数をかけ合わせた値;
  double after = 0; // フィルタ処理後の値
  ...
  printf("フィルタ処理する画素の座標を入力して下さい\n");
  printf("水平座標:");
  scanf("%d", &point_H);
  printf("垂直座標:");
  scanf("%d", &point_V);

  /* 上下左右の端にある画素が選択された場合、処理をしない */
  if(point_H <= 0 || point_H >= width || point_V <= 0 || point_V >= height){
      printf("---- この座標は画像の端に位置するためフィルタ処理ができません\n");
      return;
  }
  ...
  for(i=-1; i<2; i++){
    for(j=-1; j<2; j++){
      printf("%d ", imgin[0][point_H+j][point_V+i]);
    }
    printf("\n");
  }
  ...
  for(i=-1; i<2; i++){
    for(j=-1; j<2; j++){
      a_imgin = imgin[0][point_H+j][point_V+i] * filter[j+1][i+1];
      after += a_imgin;
    }
  }
